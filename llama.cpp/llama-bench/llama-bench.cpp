...
static cmd_params parse_cmd_params(int argc, char ** argv) {
    ...
    std::vector<int> n_gpu_layers_transformed ((params.n_gpu_layers).size(), 0);
    
    for(int iter = 0; iter < (params.n_gpu_layers.size()); iter++)
        n_gpu_layers_transformed[iter] = llamafile_gpu_layers((params.n_gpu_layers)[iter]);

    params.n_gpu_layers = n_gpu_layers_transformed;

    if (FLAG_gpu == LLAMAFILE_GPU_DISABLE) {
        (params.n_gpu_layers).assign(n_gpu_layers_transformed.size(), 0);
    }
...
const bool        test::cuda         = false;
const bool        test::opencl       = !!ggml_cpu_has_clblast();
const bool        test::vulkan       = !!ggml_cpu_has_vulkan();
const bool        test::kompute      = !!ggml_cpu_has_kompute();
const bool        test::metal        = !!ggml_cpu_has_metal();
const bool        test::gpu_blas     = false;//!!ggml_cpu_has_gpublas();
const bool        test::blas         = false;
